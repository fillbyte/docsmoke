...
docker run --rm -v "$PWD:/work" -w /work \
    ghcr.io/fillbyte/docsmoke:1.0.4 scan README.md docs examples
```

Use `:latest` for convenience, `:1` for the moving stable major line,
or `:1.0.4` for a fully pinned container.
...
`@v1.0.4` for an exact action revision.
