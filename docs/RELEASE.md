...
- **Major action tags** such as `v1` are moving compatibility tags for
  ...
  `1.x` release, not specifically to `v1.0.3`.

For example, after `v1.0.3` is released, both `v1.0.3` and `v1` point
...
uses: fillbyte/docsmoke@v1.0.3
```

This follows GitHub's documented action-maintenance guidance: create
semantic version tags such as `v1.0.3` and keep major tags such as `v1`
...
So `v1` does not mean "the original `v1.0.3` forever." It means "the
