...
- **Package release tags** such as `v1.0.1` are immutable release
...
- **Major action tags** such as `v1` are moving compatibility tags for
  ...
  `1.x` release, not specifically to `v1.0.1`.

For example, after `v1.0.1` is released, both `v1.0.1` and `v1` point
...
uses: fillbyte/docsmoke@v1.0.1
```

This follows GitHub's documented action-maintenance guidance: create
semantic version tags such as `v1.0.1` and keep major tags such as `v1`
current with the latest compatible release.

## Why `1`, `1.0`, `1.0.1`, `v1`, and `v1.0.1` all exist
...
| ------- | ------- | ------- |
| PyPI package | `1.0.1` | Exact Python package version installed by `pip` or `pipx`. |
| GitHub Release tag | `v1.0.1` | Exact release commit and assets. The `v` prefix is a Git tag convention. |
| GitHub Action tag | `v1` | Moving major tag for the latest compatible `1.x` action release. |
| GHCR image tag | `1.0.1` | Exact container image for one release. |
...
So `v1` does not mean "the original `v1.0.1` forever." It means "the
...
- **GHCR**: container tags for `latest`, major, major/minor, and exact
  version, for example `latest`, `1`, `1.0`, and `1.0.1`.
...
tag. Exact tags such as `v1.0.1` should only be moved to recover from a
